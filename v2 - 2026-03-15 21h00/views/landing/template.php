...
<head>
    ...
    <style>
        ...
        .nav-auth a.nav-auth-login  { color:#fff; font-size:.85rem; opacity:.8; transition:opacity .2s; }
        .nav-auth a.nav-auth-login:hover { opacity:1; }
        .nav-auth a.nav-auth-btn {
            background:#e63946; color:#fff; padding:8px 20px; border-radius:6px;
            font-size:.85rem; font-weight:600; text-decoration:none; transition:background .2s;
        }
        .nav-auth a.nav-auth-btn:hover { background:#c1121f; }
        .nav-auth .nav-auth-user {
            display:flex; align-items:center; gap:10px; color:#fff; font-size:.85rem;
        }
        .nav-auth .nav-auth-user span { opacity:.8; }
        .nav-auth .nav-auth-user a {
            background:#222; color:#fff; padding:6px 14px; border-radius:6px;
            font-size:.8rem; text-decoration:none; border:1px solid #444; transition:background .2s;
        }
        .nav-auth .nav-auth-user a:hover { background:#333; }
        .nav-auth .nav-auth-user a.dash-btn {
            background:#e63946; border-color:#e63946;
        }
        .nav-auth .nav-auth-user a.dash-btn:hover { background:#c1121f; }
    </style>
</head>
<body>
        ...
        <div class="nav-auth">
            <?php if (Session::isLoggedIn()): ?>
                <div class="nav-auth-user">
                    <span>👋 <?= htmlspecialchars(Session::get('client_prenom')) ?></span>
    ...
    <footer>
        <div class="footer-grid">
            <div class="footer-brand">
                <img src="<?= SITE_URL ?>/assets/img/ssp.png" alt="SannaStudio SSP" class="footer-main-logo" loading="lazy">
